...
private:
  ...
  const double bc;
public:
  Mintegrand(double at_, double bt_, double ac_, double bc_) : at(at_), bt(bt_), ac(ac_), bc(bc_) {}
  double operator()(const double& x) const
  {
    return R::pbeta(x, at, bt, true, false ) * R::dbeta(x, ac, bc, 0);
  }
};
// [[Rcpp::export]]
double prob_pt_lt_pc_Rcpp(double ac, double bc, double at, double bt)
{
  Mintegrand f(at, bt, ac, bc);
  double err_est;
  int err_code;
...
List MEM_binary(double r_ctrl, double n_ctrl, 
  ...
  for (int s = 1; s < Mall; s++) {
    ...
    draws_diff[s] = draws_pc[s]-draws_pt[s]; 
    
    draws_rcstar[s] = rbinom(1, nstar, draws_pc[s])[0];
    draws_rtstar[s] = rbinom(1, nstar, draws_pt[s])[0];
    
    double ac_post = 1.0+draws_rcstar[s]+r_ctrl; 
    double at_post = 1+draws_rtstar[s]+r_trt;
    double bc_post = 1+nstar+n_ctrl-(draws_rcstar[s]+r_ctrl); 
    double bt_post = 1+nstar+n_trt-(draws_rtstar[s]+r_trt);
    
    double ppp_val = prob_pt_lt_pc_Rcpp(ac_post, bc_post, at_post, bt_post); 
    draws_ppp[s] = ppp_val;
  }
  ...
  //result["pc"] = draws_pc ; 
  //result["pt"] = draws_pt;
  //result["diff"] = draws_diff;
  //result["rcstar"] = draws_rcstar ; 
  //result["rtstar"] = draws_rtstar ; 
  result["PPP"] = draws_ppp ;
