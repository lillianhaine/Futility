...
List SS_MIX(NumericVector ps_trial,  
  ...
  for (int s = 1; s < Mall; s++) {
    ...
    double shape_ct = ((nct)/2.0) + a_tauct;
    double rate_ct =  b_tauct + (0.5 * (sum(pow( (ps_trial-muct_cur), 2)) ) );
    ...
    double mean_muct = tauct_new*(nct*mean_ct)/(sd_muct+tauct_new*(nct));
    double prec_muct = sd_muct+ ( tauct_new*(nct) );
